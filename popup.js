document.addEventListener("DOMContentLoaded", function () {
    function t(t, e) {
        const n = t.results,
            c = document.getElementById("results-container");
        if (((c.innerHTML = ""), 0 === n.length)) return void (c.innerHTML = '<div style="padding: 10px">Không tìm thấy kết quả nào.</div>');
        const r =
        ...
        (i.className = "header"),
            ...
            document.querySelector(".btn.copy-titles").addEventListener("click", s),
            document.querySelector(".btn.copy-urls").addEventListener("click", a);
        ...
        n.forEach((t, e) => {
            ...
            (o.className = "result-item"),
                (o.innerHTML = `\n                <p><b>#${n}</b>: <a href="${new URL(t.url).origin}" target="_blank" class="result-site">${
                    t.site
                }</a></p>\n                <a href="${t.url}" target="_blank" class="result-title">${
                    t.title
    ...
    function e(t) {
        ...
        const o = new URLSearchParams(new URL(t).search),
            s = parseInt(o.get("start")) || 0,
            a = Math.floor(s / 10) + 1,
            c = document.querySelector(".pagination-container");
        if ((c && c.remove(), a > 1)) {
            const o = document.createElement("button");
            (o.className = "btn-pagination prev"), (o.textContent = "Prev"), o.addEventListener("click", () => n(a - 1, t)), e.appendChild(o);
        }
        const r = document.createElement("button");
        (r.className = "btn-pagination next"),
            (r.textContent = "Next"),
            r.addEventListener("click", () => n(a + 1, t)),
            e.appendChild(r),
    ...
    function n(n, o) {
        ...
        a.set("start", 10 * (n - 1)),
            (s.search = a.toString()),
            chrome.tabs.update({ url: s.toString() }, function (n) {
                chrome.tabs.onUpdated.addListener(function o(a, c) {
                    a === n.id &&
                        "complete" === c.status &&
                        (chrome.tabs.sendMessage(a, { action: "extractSERPResults" }, function (n) {
                            n
                                ? (t(n, s.toString()), e(s.toString()))
